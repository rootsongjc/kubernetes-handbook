...
在这种情况下，”pods”是命名空间资源，而”log”是 pods 的子资源。为了在 RBAC 角色中表示出这一点，我们需要使用斜线来划分资源 与子资源。如果需要角色绑定主体读取 pods 以及 pod log，你需要定义以下角色：
...
通过`resourceNames`列表，角色可以针对不同种类的请求根据资源名引用资源实例。当指定了`resourceNames`列表时，不同动作 种类的请求的权限，如使用”get”、”delete”、”update”以及”patch”等动词的请求，将被限定到资源列表中所包含的资源实例上。例如，如果需要限定一个角色绑定主体只能”get”或者”update”一个 configmap 时，你可以定义以下角色：
...
- 如果你的 API Server 在运行时启用了非安全端口（`--insecure-port`），你也可以通过这个没有施行认证或者授权的端口发送角色或者角色绑定请求。
...
这一点允许你根据需要向特定服务账户授予特定权限。细粒度的角色绑定将提供更好的安全性，但需要更多精力管理。更粗粒度的授权可能授予服务账户不需要的 API 访问权限（甚至导致潜在授权扩散），但更易于管理。
...
3. 为命名空间中所有的服务账户授予角色
   ...
   如果你希望命名空间内的所有应用程序都拥有同一个角色，无论它们使用什么服务账户，你可以为该命名空间的服务账户用户组授予角色。
...
4. 对集群范围内的所有服务账户授予一个受限角色（不鼓励）
   ...
   如果你不想管理每个命名空间的权限，则可以将集群范围角色授予所有服务账户。
...
5. 授予超级用户访问权限给集群范围内的所有服务账户（强烈不鼓励）
   ...
   如果你根本不关心权限分块，你可以对所有服务账户授予超级用户访问权限。
...
当以日志级别为 2 或更高（`--v = 2`）运行时，你可以在 API Server 日志中看到 RBAC 拒绝请求信息（以 `RBAC DENY:` 为前缀）。你可以使用该信息来确定哪些角色需要授予哪些用户，用户组或服务账户。一旦 [授予服务账户角色](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#service-account-permissions)，并且服务器日志中没有 RBAC 拒绝消息的工作负载正在运行，你可以删除 ABAC 授权器。

### 宽泛的 RBAC 权限

你可以使用 RBAC 角色绑定来复制一个宽泛的策略。
