...
Istio 社区推出的 [Ambient Mesh](https://lib.jimmysong.io/blog/introducing-ambient-mesh/)，本质上是为了分离 sidecar proxy（Envoy）中的 L4 和 L7 功能，让一部分仅需要**安全功能**的用户可以最小阻力（低资源消耗、运维成本）地使用 Istio。这篇文章将向你介绍 Ambient Mesh，为什么需要它，以及它目前的局限性。

## 为什么推出 Ambient Mesh？

Istio 在推出之初就使用 sidecar 模式的数据平面，在网格的每个 pod 或 VM 中注入一个 sidecar proxy，这样为用户

Istio 最早支持的数据平面模式是 sidecar，也是目前大多数 service mesh 选择的数据平面模式；而后推出的 proxyless 模式，这也是简化 Istio 数据平面的一种模式。

## 使用 Ambient 模式的益处

这种数据平面模式简化了对数据平面的运维，扩大应用兼容性并降低基础设施成本。

用户可以选择将 Ambient Mesh 集成到其基础设施的网格数据平面，放弃 sidecar 代理，同时保持 Istio 的零信任安全、遥测和流量管理等核心功能。该模式目前还是预览版，Istio 社区准备在未来几个月内将其推向生产就绪。
